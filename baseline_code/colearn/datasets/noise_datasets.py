        ...
        for i, idx in enumerate(indices):
            if i < self.percent * len(self.data):
                self.targets[idx] = np.random.randint(
                    low=self.min_target, high=self.max_target + 1, dtype=np.int32
                )
            ...
            for i in self.asym_trans.keys():
                ...
                for j, idx in enumerate(indices):
                    if j <= self.percent * len(indices):
                        self.targets[idx] = (
                            self.asym_trans[i] if i in self.asym_trans.keys() else i
                        )
        del target_copy
...
class NoiseCIFAR10(CIFAR10, NoiseDataset):
    def __init__(
        ...
        # dora modify to adapt our dataset
        ...
        data_name = "%s_inc_data.npy" % (dataset_name)
        label_name = "%s_inc_labels.npy" % (dataset_name)

        data_path = os.path.join(data_dir, data_name)
        label_path = os.path.join(data_dir, label_name)

        d0_data_name = "%s_train_data.npy" % (dataset_name)
        d0_label_name = "%s_train_labels.npy" % (dataset_name)

        d0_data_path = os.path.join(data_dir, d0_data_name)
        d0_label_path = os.path.join(data_dir, d0_label_name)

        inc_data = np.load(data_path)
        d0_data = np.load(d0_data_path)

        self.data = np.concatenate([inc_data, d0_data], axis=0)

        inc_label = np.load(label_path)
        d0_label = np.load(d0_label_path)
        self.targets = np.concatenate([inc_label, d0_label], axis=0)
...
class NoiseCIFAR100(CIFAR100, NoiseDataset):
    def __init__(
        ...
        NoiseDataset.__init__(
            self, noise_type=noise_type, asym_trans=None, percent=percent
        )
