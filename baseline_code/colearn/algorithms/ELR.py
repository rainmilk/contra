...
class ELR:
    ...
    def set_optimizer(self, dataset, num_classes, config):
        ...
        #         self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[40, 80], gamma=0.01,
        #                                                               verbose=True)
        ...
        #     self.model_scratch = get_model(config['model1_type'], input_channel, num_classes, device)
        #     self.optimizer = torch.optim.SGD(self.model_scratch.parameters(), lr=lr, weight_decay=wd)
        #     self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[80, 120], gamma=0.01,
        #                                                           verbose=True)
        #     N = 50000
        #
