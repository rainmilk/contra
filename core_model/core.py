...
def iterate_repair_model(
    ...
    for label in list(set(aux_labels)):
        ...
        top_idx = np.argpartition(distances, selected_top_conf_num)[selected_top_conf_num:]
    ...
    mix_labels_onehot = np.concatenate([aux_labels_onehot, selected_labels_onehot, centroid_probs_sharpen], axis=0)
    working_mix_dataloader_shuffled = mix_up_data(mix_data, mix_labels_onehot, mix_data, mix_labels_onehot, mean, std, args.batch_size)
    ...
    teacher_mix_dataloader_shuffled = mix_up_data(mix_data, mix_labels_onehot, mix_data, mix_labels_onehot, mean, std, args.batch_size)
    model_train(
...
def iterate_adapt_model(
    ...
    # (1) 构造 Dts: Dt={Xts, Pts}, Pt = Mt(Xts)
    test_predicts_new, test_probs_new, _ = teacher_model_forward(test_dataloader, teacher_model)
    ...
    ts_mixed_dataloader_shuffled_new = mix_up_data(
        test_data, test_probs_new_sharpen, aug_data, aug_probs, mean, std, args.batch_size
    )
...
def execute(args):
    ...
    # )
    working_opt = optim.SGD(working_model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=5e-4)
    working_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(working_opt, T_max=200)
    ...
    # )
    teacher_opt = optim.SGD(lip_teacher_model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=5e-4)
    teacher_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(teacher_opt, T_max=200)
    ...
    if os.path.exists(lip_teacher_model_path):
        ...
        train_data, train_labels, train_dataloader = (
            get_dataset_loader(args.dataset, 'train', data_dir, mean, std, args.batch_size,
                               num_classes=num_classes, drop_last=True, shuffle=True))

        lip_teacher_model_dir = os.path.dirname(lip_teacher_model_path)
        model_train(train_dataloader, lip_teacher_model, teacher_opt, teacher_lr_scheduler, teacher_criterion, alpha,
                    args, save_path=lip_teacher_model_dir)
    ...
    print("---------------------working model test before------------------------------")
    working_model_test_before = model_test(test_labels, test_dataloader, working_model, device=device)
    print("---------------------teacher model test before------------------------------")
    teacher_model_test_before = model_test(test_labels, test_dataloader, lip_teacher_model, device=device,
                                           teacher_model=True)
    ...
    working_model_after_repair = model_test(test_labels, test_dataloader, working_model, device=device)
    teacher_model_after_repair = model_test(test_labels, test_dataloader, lip_teacher_model, device=device,
                                            teacher_model=True)
    ...
    working_model_after_adapt = model_test(test_labels, test_dataloader, working_model, device=device)
    teacher_model_after_adapt = model_test(test_labels, test_dataloader, lip_teacher_model, device=device,
                                           teacher_model=True)
