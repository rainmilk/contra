    ...
    def __init__(
        ...
        if mode == "all":
            ...
            for kk in range(num_class):
                class_ind[kk] = [
                    i for i, x in enumerate(train_imgs) if self.train_labels[x] == kk
                ]

            ## Creating the Class Balance
            for i in range(num_class):
                sorted_indices = np.argsort(
                    probability[class_ind[i]]
                )  ##  Sorted indices for each class
                class_indices = np.array(class_ind[i])  ##  Class indices
                size1 = len(class_indices)
                try:
                    pred_idx[size_pred : size_pred + class_len] = class_indices[
                        sorted_indices[0:class_len].cpu().numpy()
                    ].squeeze()
                    size_pred += class_len
                except:
...
class clothing_dataloader:
    def __init__(self, root, batch_size, warmup_batch_size, num_batches, num_workers):
