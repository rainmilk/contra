...
parser.add_argument(
    "--lr", "--learning_rate", default=0.005, type=float, help="initial learning rate"
)  ## Set the learning rate to 0.005 for faster training at the beginning
parser.add_argument("--alpha", default=0.5, type=float, help="parameter for Beta")
parser.add_argument(
    "--lambda_c", default=0.025, type=float, help="weight for contrastive loss"
)
...
parser.add_argument(
    "--data_path", default="./data/Clothing1M", type=str, help="path to dataset"
)
...
parser.add_argument(
    "--resume", default=False, type=bool, help="Resume from the warmup checkpoint"
)
...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader):
    ...
    for batch_idx, (
        ...
        sys.stdout.write("\r")
        sys.stdout.write(
            "%s: | Epoch [%3d/%3d] Iter[%3d/%3d]\t Labeled loss: %.2f  Contrative Loss:%.4f"
            % (
                args.dataset,
                epoch,
                args.num_epochs,
...
for epoch in range(0, args.num_epochs + 1):
    ...
    if epoch < warm_up:
        ...
        SR = (
            torch.sum(prob2 < threshold).item() / prob2.size()[0]
        )  ## Calculate the Ratio of clean samples

        for i in range(nb_repeat):
            print("\n\nTrain Net1")
            labeled_trainloader, unlabeled_trainloader = loader.run(
                SR, "train", prob=prob2, paths=paths2
            )  ## Uniform Selection
            train(
                epoch,
                net1,
        ...
        net1.load_state_dict(
            torch.load(os.path.join(model_save_loc, "%s_net1.pth.tar" % args.id))
        )
        ...
        for i in range(nb_repeat):
            print("\nTrain Net2")
            labeled_trainloader, unlabeled_trainloader = loader.run(
                SR, "train", prob=prob1, paths=paths1
            )  ## Uniform Selection
            train(
                epoch,
                net2,
    ...
    net2.load_state_dict(
        torch.load(os.path.join(model_save_loc, "%s_net2.pth.tar" % args.id))
    )
    log.flush()
